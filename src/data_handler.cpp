...
void data_handler::read_feature_labels(std::string path)
{
	...
	if(f)
	{
		...
		printf("Successfully read and stored lables.\n");
...
void data_handler::split_data()
{
	...
	int valid_size = data_array->size() - training_size -test_size;
...
void data_handler::count_classes()
{
	...
	for(unsigned i = 0; i < data_array->size(); i++)
	{
		if(class_map.find(data_array->at(i)->get_label()) == class_map.end())
		{
			...
			label_vector.push_back(data_array->at(i)->get_label());
			count++;
		}
...
uint8_t data_handler::get_label(int index)
{
	return label_vector[index];
}
/*****
int main()
{
